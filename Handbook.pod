...
L<http://www.slideshare.net/sharifulin/mojo-en>.
...
=head1 TRANSACTION WALK-THROUGH

I<The Pill-Cam View (tm)>

B<XXX Note to self>: A step-by-step walk-through of processing of a
single transaction with Daemon and Routes (and perhaps a sprinkle of
Static), complete with actions, handlers, helpers, templates etc. En
lieu of understanding the architecture, basically. Should have helped writing
the previous section. But is probably now pointless.

L<Mojo::IOPoll> accepts the TCP connection.

One of L<Mojo::Server> processes HTTP connection in conjunction with
the transaction (C<$tx>). Transaction encapsulates request (C<$req>)
and response (C<$res>) objects, accessible via its attributes
(C<<$tx->??>>) and serves as the state machine for a transaction,
meaning that only the combination of the three components represent a
full HTTP 1.1 server.

XXX Note on WebSockets here.
